...
reporting metrics for each strategy independently within the same experiment.
"""
...
def analyze_experiment_logs(
    ...
    UPDATED: Now supports Multi-Strategy Branching. It iterates through the primary
    solution strategy AND any alternative strategies (like 'base_filtering') found
    in `steps_alternatives`.
    ...
    for exp_name, query_logs in all_experiments_logs.items():
        ...
        # Note: If we want to report simple Pass@K for Group Consistency, we can remove this skip,
        # as orchestration.py now populates 'solution_attempts' for groups too.
        # For now, we keep it separate to avoid duplicates if the user runs the specific analysis.
        if exp_config.get("APPLY_GROUP_CONSISTENCY_SELECTION", False):
             # We allow standard analysis for Group Consistency to get a baseline Pass@K metric
             pass
        ...
        for loop_idx, log in enumerate(tqdm(query_logs, desc=f"Evaluating {exp_name}")):
            ...
            # B. Alternative Strategies (Branching location)
